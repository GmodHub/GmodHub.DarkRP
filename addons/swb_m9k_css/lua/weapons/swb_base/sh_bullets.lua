...
function SWEP:FireBullet(damage, cone, bullets)
	...
	local clumpspreads = self.ClumpSpreads
	...
	for i = 1, bullets do
		...
		if self.ClumpSpread and self.ClumpSpread > 0 then
			Dir2 = Dir + (clumpspreads and clumpspreads[i] or Vector(math.Rand(-1, 1), math.Rand(-1, 1), math.Rand(-1, 1))) * self.ClumpSpread
		end
		...
		if trace.Hit and (not trace.HitSky) and (not NoPenetration[trace.MatType]) and (not (IsValid(trace.Entity) and trace.Entity.NoBulletPenetration)) then
