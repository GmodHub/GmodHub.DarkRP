...
local ScopeTex = GetRenderTarget("swb_scope_texture", ScrW(), ScrH(), false)

function SWEP:PostDrawViewModel(vm)
	if self.ScopeOverrideMaterialIndex then
		vm:SetSubMaterial(self.ScopeOverrideMaterialIndex)
	end
end

_swb_scope_mats = _swb_scope_mats or {}
local mats = _swb_scope_mats

function SWEP:PreDrawViewModel(vm, ply, wep)
	CT = UnPredictedCurTime()
	local needs_scope_render = self.dt.State == SWB_AIMING and self.ScopeOverrideMaterialIndex
	if needs_scope_render then
		local ScopeMat = mats[self.ClassName]
		if (not ScopeMat) then
			local tex = Material(self:GetMaterials()[self.ScopeOverrideMaterialIndex + 1]):GetTexture("$basetexture")
			local mul_x, mul_y = self.ScopeFlipX and -1 or 1, self.ScopeFlipY and -1 or 1
			local w_ratio = tex:GetMappingWidth() / ScrW()
			local h_ratio = tex:GetMappingHeight() / ScrH()

			if (w_ratio > h_ratio) then
				mul_y = mul_y * w_ratio / h_ratio
			else
				mul_x = mul_x * h_ratio / w_ratio
			end

			mats[self.ClassName] = CreateMaterial("swb_scope_material______"..self:GetClass(), "UnlitGeneric", {
				["$basetexture"] = ScopeTex:GetName(),
				["$vertexcolor"] = 0,
				["$vertexalpha"] = 0,
				["$nolod"] = 1,
				["$model"] = 1,
				["$basetexturetransform"] = string.format("center 0.5 0.5 scale %f %f rotate 0 translate %f %f",
					1 / mul_x,
					1 / mul_y,
					self.TranslateX or 0,
					self.TranslateY or 0
				)
			})

			ScopeMat = mats[self.ClassName]
		end

		vm:SetSubMaterial(self.ScopeOverrideMaterialIndex, "!"..ScopeMat:GetName())
	end
	...
	if needs_scope_render then
		local a = self.AimAng
		local _, vmang = self:GetViewModelPosition(vector_origin * 1, angle_zero * 1)

		local scrh = ScrH()
		local scrw = ScrW()
		local pct = math.Clamp((self.AimTime - CurTime()) / 0.25, 0, 1)
		render.PushRenderTarget(ScopeTex)
			render.Clear(0, 0, 0, 255, true, true)
			render.RenderView {
				origin = EyePos(),
				angles = EyeAngles() + Angle(a.x, a.y, a.z) - vmang,
				drawviewmodel = false,
				fov = self.AdjustableZoom and (self.ScopeZoomAmount or self.MaxZoom) or self.ScopeFOV,
				x = 0,
				y = 0,
				w = ScrW(),
				h = ScrH()
			...
			cam.Start2D()
				if (self.ScopeTexture) then
					...
					surface.DrawTexturedRect((self.ScopeTextureX or 0), (self.ScopeTextureY or 0), scrw, scrh)
				elseif (self.DrawScope) then
					self:DrawScope(scrw, scrh)
				else
					surface.SetDrawColor(0, 0, 0, 255)
					surface.DrawRect(0, scrh / 2 - 3, scrw, 7)
					surface.DrawRect(scrw / 2 - 3, 0, 7, scrh)
				end

				if self.AdjustableZoom then
					local y = (scrh * 0.5) - ((self.ZoomSteps * 0.5) * 50)

					surface.SetFont('ui.40')
					for i = 1, self.ZoomSteps do
						local step = i - 1

						if self._ZoomStep and (step == self._ZoomStep) then
							surface.SetDrawColor(225, 225, 225, 150)
						...
						local x, y = (scrw * 0.5) + (scrh * 0.275), y + (step * 52)

						surface.DrawRect(x, y, 50, 50)
						surface.SetTextPos(x + 5, y)
						surface.DrawText(i .. 'x')
					end
					...
					if IsValid(ent) and isplayer(ent) then
						...
						local text = ent:Name() .. ' - ' .. math.ceil(ent:GetPos():Distance(LocalPlayer():GetPos())/25) .. 'm'

						local w, h = surface.GetTextSize(text)

						surface.SetTextPos((scrw * 0.5) - (w * 0.5), scrh * 0.75)
						surface.DrawText(text)
					end
				end

				surface.SetDrawColor(0, 0, 0, 255 * pct)
				surface.DrawRect(0, 0, scrw, scrh)
			cam.End2D()

		render.PopRenderTarget()

		if self.ScopedBlur then
			if (not blur) then
			...
			cam.Start2D()
				...
				surface.SetDrawColor(255, 255, 255, (1 - pct) * 255)
