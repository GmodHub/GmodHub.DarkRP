...
local swb_scope_sensitivity_multiplier = CreateConVar("swb_scope_sensitivity_multiplier", "2", FCVAR_ARCHIVE)
...
function SWEP:CalcView(ply, pos, ang, fov)
	...
	if not self.HasScope and self.dt.State == SWB_AIMING then
...
function SWEP.CreateMove(move)
	...
	if IsValid(wep) and wep.SWBWeapon and wep.dt and wep.dt.State == SWB_AIMING and wep.AimBreathingEnabled then
		CT = CurTime()
...
function SWEP:AdjustMouseSensitivity()
	if self.dt.State == SWB_RUNNING and self.RunMouseSensMod then
		return self.RunMouseSensMod
	end

	CT = CT or CurTime()

	if self.dt.State == SWB_AIMING and CT > self.AimTime or self.InstantDissapearOnAim then
		if (self.OverrideAimMouseSens) then
			return self.OverrideAimMouseSens
		elseif (self.MaxZoom) then
			local dist = math.tan(math.rad(LocalPlayer():GetFOV() / 2))
			local scoped_dist = math.tan(math.rad((self.AdjustableZoom and (self.ZoomAmount or self.MaxZoom) or self.ScopeFOV) / 2))
			return scoped_dist / dist * swb_scope_sensitivity_multiplier:GetFloat() / 2
		end
	end
