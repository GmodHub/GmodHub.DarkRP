...
function SWEP:DrawHUD()
	...
	if self.CrosshairEnabled and self.Owner == LocalPlayer() then
		...
		self.CrossAmount = Lerp(FT * 10, self.CrossAmount, (self:GetCurrentCone() * 350) * (90 / (math.Clamp(GetConVarNumber("fov_desired"), 75, 90) - self.CurFOVMod)))

		if self.ClumpSpread then
			size = math.Clamp(self.CrossAmount * 0.8, 0, math.ceil(self.ClumpSpread * 2000))

			surface.DrawCircle(x2, y2, size + 2, 0, 0, 0, self.CrossAlpha)
			surface.DrawCircle(x2, y2, size-2, 0, 0, 0, self.CrossAlpha)
			surface.DrawCircle(x2, y2, size, 255, 255, 255, self.CrossAlpha)
		end
