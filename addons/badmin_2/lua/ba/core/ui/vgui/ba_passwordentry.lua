...
net.Receive("ba.PasswordRequest", function(len)
	...
	if (!isReset) then
		...
		if (hasResetKey) then
			...
			local lbl = ui.Create("DLabel", function(self)
				self:SetText("Enter reset code:")
				self:SizeToContents()
				self:SetPos(5, 29)
			end, fr)

			local lbl2 = ui.Create("DLabel", function(self)
				self:SetText("Choose new password:")
				self:SizeToContents()
				self:SetPos(5, 80)
			end, fr)
			...
			local lbl = ui.Create("DLabel", function(self)
				self:SetText("Create a password:")
				self:SizeToContents()
				self:SetPos(5, 29)
			end, fr)
	...
	submit.Think = function(self)
		if (isReset) then
			if (hasResetKey) then
				...
				if (#code == 0) then
					self:SetText("Enter the reset code")
