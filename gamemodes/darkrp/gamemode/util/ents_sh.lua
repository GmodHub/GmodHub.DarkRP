...
function ENTITY:IsIllegalWeapon() -- TODO: ADD ENT.IllegalWep var to all illegal sweps
	return wep_classes[self:GetClass()] or (string.sub(self:GetClass(), 0, 3) == 'swb') or (string.sub(self:GetClass(), 0, 6) == 'knife_')
end
function rp.IsIllegalWeapon(class)
	return wep_classes[class] or (string.sub(class, 0, 3) == 'swb') or (string.sub(class, 0, 6) == 'knife_')
end
