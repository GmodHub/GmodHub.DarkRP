...
function _R.Legs:ShouldDraw()
	return self:IsValid() and
			...
			!LocalPlayer():InVehicle() and
			GetViewEntity() == LocalPlayer() and
			!LocalPlayer():ShouldDrawLocalPlayer() and
			!IsValid(LocalPlayer():GetObserverTarget()) and
...
function _R.Legs:Render()
	...
	if (self.Entity:GetModel() != LocalPlayer():GetFixedModel()) then
		self.Entity:SetModel(LocalPlayer():GetFixedModel())
	end
...
hook("UpdateAnimation", "Legs:UpdateAnimation", function(ply, vel, groundSpeed)
...
hook("PostDrawTranslucentRenderables", "Legs:Render", function()
	if !LocalPlayer().Legs or (LocalPlayer():EyeAngles().pitch < 50) then
		return
	end
...
concommand.Add("resetlegs", function()
	SafeRemoveEntity(LocalPlayer().Legs.Entity)
	LocalPlayer().Legs = nil
end)