...
function PANEL:Init()
	...
	self.ModelPanel.DrawModel = function(self)
		...
		if self.Apparel then
			for k, v in pairs(self.Apparel) do
				rp.hats.Render(self.Entity, rp.hats.List[v])
			end
		end
	end
...
function PANEL:SetApparel(inf)
	self.ModelPanel.Apparel = inf
end

function PANEL:SetOutfit(outfit)
	self.Outfit = outfit
	self.ModelPanel.Entity:SetOutfit(outfit)
end
