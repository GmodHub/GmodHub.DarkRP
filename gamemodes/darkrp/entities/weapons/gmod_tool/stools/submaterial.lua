...
TOOL.Name = "#tool.submaterial.name"
...
if CLIENT then
	...
	language.Add( "tool.submaterial.desc", "Overrides submaterials of an object" )
	language.Add( "tool.submaterial.left", "Set default material")
	language.Add( "tool.submaterial.right", "Apply material")
	language.Add( "tool.submaterial.0", "Wheel Up/Down: Select target submaterial")
	language.Add( "tool.submaterial.reload", "Copy material")
...
local function SetSubMaterial( Player, Entity, Data )
	...
	if ( SERVER ) then
		...
		for i=0,MatCount-1 do
			...
			if Data[si] and ((string.lower(Data[si]) == "pp/copy" ) or Data[si] == "" or !list.Contains( "OverrideMaterials", Data[si])) then
				Data[si]=nil
			end
...
---- Thx wire_adv dev again...
	...
	hook.Add( "SuppressWeaponSwitcher", "submat_tool_playerbindpress", hookfunc )
--------------------------------------------------
	...
	function TOOL:DrawHUD( )
		if IsValid(self.AimEnt) and self.AimEnt:IsProp() then
