TOOL.Category		= "VIP+"
TOOL.Name			= "#Tool.easyprecision.name"
TOOL.Command		= nil
TOOL.ConfigName		= ""
...
if (CLIENT) then
	language.Add("Tool.easyprecision.name", "Easy Precision")
	language.Add("Tool.easyprecision.desc", "Accurately positions objects")
	language.Add("Tool.easyprecision.0", "Primary: Move/Apply | Secondary: Push | Reload: Pull")
	language.Add("Tool.easyprecision.1", "Target the second item. If enabled, this will move the first item.  (Swap weps to cancel)")
	language.Add("Tool.easyprecision.2", "Rotate enabled: Turn left and right to rotate the object (Hold reload or alt-fire for other rotation directions!)")
...
--[[	Modifications
	Line 756	--	Rotation clamp & division by 0 prevention
	Line 810	--	Snap Distance clamp
	Line 848	-- 	Rotation clamp & division by 0 prevention
	Line 1094	--	Snap Distance clamp
	Line 1102	-- 	Snap Distance clamp
	Line 1108	-- 	Rotation clamp & division by 0 prevention
	Line 1211	--	Nudge Amount Clamp (Either edit it to your liking or get rid of it)
]]

--[[
	Convienence function to get rid of all of if (self:GetClientNumber('mode')) calls
]]
function TOOL:GetToolMode(default)
	return self:GetClientNumber("mode", default)
end
--[[---------------------------------------------------------------------------------------------]]--
--[[
	Clamp the Snap Distance value (if they set it to some ridiculous number and apply it
	 with any of the tool modes, it will instantly crash the server.

	You may or may not want to decrease these values even more to prevent players from moving props way into the sky
	 or under the ground.
]]
function TOOL:GetSnapDistance()
	return math.Clamp(self:GetClientNumber("offset"), -10000, 10000)
end
--[[---------------------------------------------------------------------------------------------]]--
--[[
	Convenience function to return a boolean of whether to apply the offset as a percent or not
	 (just shortens a few ugly looking if (blah blah blah) statements
]]
function TOOL:GetSnapPercent()
	return self:GetClientNumber("offsetpercent") == 1
end
--[[---------------------------------------------------------------------------------------------]]--
--[[
	Clamps the Nudge Distance so players aren't nudging props all the way across the map with one application.

	You may want to disable this entirely to prevent players from nudging their props through walls!
]]
function TOOL:GetNudgeDistance()
	return math.Clamp(self:GetClientNumber("nudge"), -100, 100)
end
--[[---------------------------------------------------------------------------------------------]]--
--[[
	Convenience function to return a boolean of whether to apply the nudge offset as a percent or not
]]
function TOOL:GetNudgePercent()
	return self:GetClientNumber("nudgepercent") == 1
end
--[[---------------------------------------------------------------------------------------------]]--
--[[
	Convenience function to return a boolean of whether to apply the rotation settings or not
]]
function TOOL:GetDoRotate()
	return self:GetClientNumber("rotate") == 1
end
--[[---------------------------------------------------------------------------------------------]]--
--[[
	Clamps the rotation amount between 0 and 360 (should never be higher or lower).

	Note that this function alone does not fix the division by 0 crash. Those problem areas
	 have been patched individually by checking if the value returned by this function is 0.
]]
function TOOL:GetRotation()
	return math.Clamp(self:GetClientNumber("rotation"), 0, 360)
end
--[[---------------------------------------------------------------------------------------------]]--
					...
					if (CurrentPhys:IsValid()) then
						...
						if (CurrentEnt:GetPhysicsObjectCount() < 2) then //not a ragdoll
							...
							elseif (mode == 4) then //weld
								...
								if (constraint) then
									undo.Create("easyprecision_Weld")
							...
							elseif (mode == 5) then //doaxis
								...
								if (constraint) then
									undo.Create("easyprecision_Axis")
									undo.AddEntity(constraint)
									undo.SetPlayer(self:GetOwner())
									undo.Finish()
									self:GetOwner():AddCleanup("constraints", constraint)
							...
							elseif (mode == 6) then //ballsocket
								...
								if (constraint) then
									undo.Create("easyprecision_Ballsocket")
									undo.AddEntity(constraint)
									undo.SetPlayer(self:GetOwner())
									undo.Finish()
									self:GetOwner():AddCleanup("constraints", constraint)
							...
							elseif (mode == 7) then //adv ballsocket
								...
								if (constraint) then
									undo.Create("easyprecision_Advanced_Ballsocket")
									undo.AddEntity(constraint)
									undo.SetPlayer(self:GetOwner())
									undo.Finish()
									self:GetOwner():AddCleanup("constraints", constraint)
							...
							elseif (mode == 8) then //slider
								...
								if (constraint0) then
									undo.Create("easyprecision_Slider")
...
function TOOL:LeftClick(trace)
	...
	if (iNum < 2) then
		...
		if (SERVER && !util.IsValidPhysicsObject(trace.Entity, trace.PhysicsBone )  && (!trace.Entity:GetParent():IsValid() || ! self:GetClientNumber("removal") == 1 || !mode == 9)) then
			if  !(mode < 4 && iNum == 0) then
				return false
			end
		end
		...
		if (SERVER && !Phys:IsValid() && (!trace.Entity:GetParent():IsValid() || !self:GetClientNumber("removal") == 1 || !mode == 9)) then
			if  !(mode < 4 && iNum == 0) then
				return false
			end
		end
	...
	if (iNum == 0) then
		...
		elseif (self:GetClientNumber("removal") == 1 && mode > 3) then//Constraint mode - Attempt to remove.
			...
			return bool
		elseif (self:GetClientNumber("Move") == 0 && mode != 3) || mode < 3 then
			if mode == 2 then
				if (CLIENT) then return true end
				//self:SetStage(2)
				//iNum = 2
				...
				if (self:GetClientNumber("rotateundo")) then
					undo.Create("easyprecision_Rotate")
						undo.SetPlayer(self:GetOwner())
				...
				--[[ CRASH PREVENTION - Return a clamped rotation and then return 0 if rotation == 0 (to prevent division by 0) ]]--
				local rotation = self:GetRotation()
				local isZero = (rotation == 0)
				...
				if (inuse == nil || inuse == self:GetOwner():GetName()) then
					...
					realdegrees, realdegreesY, realdegreesZ = 0, 0, 0
					lastdegrees =  (isZero and 0) or -((rotation/2) % rotation)
					lastdegreesY, lastdegreesZ = lastdegrees, lastdegrees
					...
					realdegrees2, realdegrees2Y, realdegrees2Z = 0, 0, 0
					lastdegrees2 = (isZero and 0) or -((rotation/2) % rotation)
					lastdegrees2Y, lastdegrees2Z = lastdegrees2, lastdegrees2
	...
	elseif (iNum == 1) then
		if (self:GetEnt(1) == self:GetEnt(2)) then
			...
			//self:SendMessage("Fool! You can't constrain something to itself")
			//self:ClearObjects()
			//return false
		end

		--[[ CRASH PREVENTION - Return a clamped Snap Distance amount to prevent crashes from ridiculously high values ]]--
		local offset = self:GetSnapDistance()
		local move   = (self:GetClientNumber("move") == 1 && mode > 2) || mode == 3
		...
		if (move && mode < 9) then//No move on parent
			...
			--[[ CRASH PREVENTION - Return a clamped rotation and then return 0 if rotation == 0 (to prevent division by 0) ]]--
			local doRotate = self:GetDoRotate()
			local rotation = self:GetRotation()
			local isZero   = (rotation == 0)

			if ((doRotate && mode != 1) || mode == 2) then //Set axies for rotation mode directions
				if (inuse == nil || inuse == self:GetOwner():GetName()) then
					inuse = self:GetOwner():GetName()
					axis = Norm2
			...
			local offsetpercent = self:GetSnapPercent()
		...
		self:ReleaseGhostEntity()
		local rotate = (self:GetDoRotate() && mode > 1 && ((self:GetClientNumber("move") == 1 && mode > 2) || mode == 3)) || mode == 2
...
function TOOL:Think()
	...
	if self:NumObjects() == 1 && mode != 2 then
		if ((self:GetClientNumber("move") == 1 && mode > 2) || mode == 3) then
			if (mode < 9) then//no move = no ghost in parent mode
				--[[ CRASH PREVENTION - Return a clamped Snap Distance amount to prevent crashes from ridiculously high values ]]--
				local offset = self:GetSnapDistance()
		...
		local rotate = (self:GetDoRotate() && mode != 1) || mode == 2
		if (SERVER && rotate) then
			...
			if(self:GetOwner():KeyDown(IN_RELOAD)) then
				if (inuse == self:GetOwner():GetName()) then
					...
					newdegrees    = realdegreesY - ((isZero and 0) or ((realdegreesY + (rotation/2)) % rotation))
					...
					newdegrees    = realdegrees2Y - ((isZero and 0) or ((realdegrees2Y + (rotation/2)) % rotation))
			...
			elseif(self:GetOwner():KeyDown(IN_ATTACK2)) then
				if (inuse == self:GetOwner():GetName()) then
					...
					newdegrees    = realdegreesZ - ((isZero and 0) or ((realdegreesZ + (rotation/2)) % rotation))
					...
					newdegrees    = realdegrees2Z - ((isZero and 0) or ((realdegrees2Z + (rotation/2)) % rotation))
				...
				if (inuse == self:GetOwner():GetName()) then
					...
					newdegrees    = realdegrees - ((isZero and 0) or ((realdegrees + (rotation/2)) % rotation))
					...
					newdegrees    = realdegrees2 - ((isZero and 0) or ((realdegrees2 + (rotation/2)) % rotation))
...
function TOOL:Nudge(trace, direction)
	...
	--[[ CLAMP NUDGE AMOUNT -- Prevent players from nudging props by obscure amounts ]]
	local offset = self:GetNudgeDistance()
	local offsetpercent = self:GetNudgePercent()
...
if CLIENT then
	...
	local function AddDefControls(Panel)
		...
		Panel:AddControl("ComboBox",
		{
			...
			CVars =
			{
				[0] = "easyprecision_offset",
				[1] = "easyprecision_forcelimit",
				[2] = "easyprecision_freeze",
				[3] = "easyprecision_nocollide",
				[4] = "easyprecision_nocollideall",
				[5] = "easyprecision_rotation",
				[6] = "easyprecision_rotate",
				[7] = "easyprecision_torquelimit",
				[8] = "easyprecision_friction",
				[9] = "easyprecision_mode",
				[10] = "easyprecision_width",
				[11] = "easyprecision_offsetpercent",
				[12] = "easyprecision_removal",
				[13] = "easyprecision_move",
				[14] = "easyprecision_physdisable",
				[15] = "easyprecision_advballsocket",
				[16] = "easyprecision_XRotMin",
				[17] = "easyprecision_XRotMax",
				[18] = "easyprecision_YRotMin",
				[19] = "easyprecision_YRotMax",
				[20] = "easyprecision_ZRotMin",
				[21] = "easyprecision_ZRotMax",
				[22] = "easyprecision_XRotFric",
				[23] = "easyprecision_YRotFric",
				[24] = "easyprecision_ZRotFric",
				[25] = "easyprecision_FreeMov",
				[26] = "easyprecision_ShadowDisable",
				[27] = "easyprecision_allowphysgun",
				[28] = "easyprecision_autorotate",
				[29] = "easyprecision_massmode",
				[30] = "easyprecision_nudge",
				[31] = "easyprecision_nudgepercent",
				[32] = "easyprecision_disablesliderfix"
			}
		})
		...
		Panel:AddControl( "Slider",  { Label	= "Push/Pull Amount",
					...
					Min		= 1,
					Max		= 100,
					Command = "easyprecision_nudge",
					Description = "Distance to push/pull props with altfire/reload"}	 ):SetDecimals(4)


		Panel:AddControl("Checkbox", { Label = "Push/Pull as Percent (%) of target's depth", Command = "easyprecision_nudgepercent", Description = "Unchecked = Exact units, Checked = takes % of width from target prop when pushing/pulling" })


		local user = LocalPlayer():GetInfoNum("easyprecision_user", 0)
		local mode = LocalPlayer():GetInfoNum("easyprecision_mode", 0)
		...
		function list:OnRowSelected(LineID, line)
			if not (mode == LineID) then
				RunConsoleCommand("easyprecision_setmode", LineID)
			end
		end
		...
		local Moving = LocalPlayer():GetInfoNum("easyprecision_move", 1) == 1 && mode > 2 && mode <= 9
		local Rotating = (LocalPlayer():GetInfoNum("easyprecision_rotate", 1) == 1 || mode == 2) && mode <= 9
		if (mode > 3 && mode <= 8) then
			Panel:AddControl("Checkbox", { Label = "Move Target? ('Easy' constraint mode)", Command = "easyprecision_move", Description = "Uncheck this to apply the constraint without altering positions." })
		end
		if (mode > 2 && mode <= 8 /*Moving || mode == 3*/) then
			Panel:AddControl("Checkbox", { Label = "Rotate Target? (Rotation after moving)", Command = "easyprecision_rotate", Description = "Uncheck this to remove the extra click for rotation. Handy for speed building." })
			...
			Panel:AddControl( "Slider",  { Label	= "Snap Distance",
					...
					Min		= 0,
					Max		= 100,
					Command = "easyprecision_offset",
					Description = "Distance offset between joined props.  Type in negative to inset when moving."}	 )
			Panel:AddControl("Checkbox", { Label = "Snap distance as Percent (%) of target's depth", Command = "easyprecision_offsetpercent", Description = "Unchecked = Exact units, Checked = takes % of width from second prop" })
		end
		if (mode > 1 && mode <= 8/*Moving || mode == 2 || mode == 3*//*Rotating && mode != 1*/) then
			Panel:AddControl("Slider",  { Label	= "Rotation Snap (Degrees)",
					...
					Min		= 0.02,
					Max		= 90,
					Command = "easyprecision_rotation",
					Description = "Rotation rotates by this amount at a time. No more guesswork. Min: 0.02 degrees "}	 ):SetDecimals(4)
		end
		if (mode != 9) then
			Panel:AddControl("Checkbox", { Label = "Freeze Target", Command = "easyprecision_freeze", Description = "Freeze props when this tool is used" })

			if (mode > 2) then
				Panel:AddControl("Checkbox", { Label = "No Collide Targets", Command = "easyprecision_nocollide", Description = "Nocollide pairs of props when this tool is used. Note: No current way to remove this constraint when used alone."  })
			end
		end
		...
		if (user >= 2 || mode == 1) then
			if ((mode > 2 && mode < 9) || mode == 1) then
				Panel:AddControl("Checkbox", { Label = "Auto-align to world (nearest 45 degrees)", Command = "easyprecision_autorotate", Description = "Rotates to the nearest world axis (similar to holding sprint and use with physgun)"  })
			end

			if (mode == 1) then
				Panel:AddControl("Checkbox", { Label = "Disable target shadow", Command = "easyprecision_ShadowDisable", Description = "Disables shadows cast from the prop"  })
			end
		end

		if (user >= 3) then
			if (mode == 1) then //apply
				Panel:AddControl("Checkbox", { Label = "Only Collide with Player", Command = "easyprecision_nocollideall", Description = "Nocollides the first prop to everything and the world (except players collide with it). Warning: don't let it fall away through the world."  })
				Panel:AddControl("Checkbox", { Label = "Disable Physics on object", Command = "easyprecision_physdisable", Description = "Disables physics on the first prop (gravity, being shot etc won't effect it)"  })
				Panel:AddControl("Checkbox", { Label = "Adv: Allow Physgun on PhysDisable objects", Command = "easyprecision_allowphysgun", Description = "Disabled to stop accidents, use if you want to be able to manually move props after phyics disabling them (may break clipboxes)."  })
			end
			if (mode == 9) then //parent
				Panel:AddControl("Checkbox", { Label = "Adv: Allow Physgun on Parented objectss", Command = "easyprecision_allowphysgun", Description = "Disabled to stop accidents, use this if you want to play with the parenting hierarchy etc."  })
			end
		end
		if (mode >= 4 && mode <= 9) then
			Panel:AddControl("Checkbox", { Label = "Constraint Removal Mode", Command = "easyprecision_removal", Description = "When enabled, attempts to undo constraints of the current mode on targeted props (Much like the reload function of other tools).  Use 'move' mode to remove nocollideall constraint." })
		end
		if (user >= 2) then
			if (mode != 2) then //todo: entire contrap rotate support
				Panel:AddControl("Checkbox", { Label = "Entire Contraption! (Everything connected to target)", Command = "easyprecision_entirecontrap", Description = "For mass constraining or removal or nudging or applying of things. Yay generic."  })
			end
		end
		...
		if (user >= 2) then
			if ((mode >= 4 && mode <= 7) || mode == 10) then //breakable constraint
				Panel:AddControl( "Slider",  { Label	= "Force Breakpoint",
						...
						Min		= 0.0,
						Max		= 50000,
						Command = "easyprecision_forcelimit",
			...
			if (mode == 5 || mode == 6 || mode == 7) then //axis or ballsocket
				Panel:AddControl( "Slider",  { Label	= "Torque Breakpoint",
						...
						Min		= 0.0,
						Max		= 50000,
						Command = "easyprecision_torquelimit",
		...
		if (mode == 5) then //axis
			Panel:AddControl( "Slider",  { Label	= "Axis Friction",
					...
					Min		= 0.0,
					Max		= 100,
					Command = "easyprecision_friction",
		...
		if (mode ==7) then //adv ballsocket
			...
			Panel:AddControl( "Slider",  { Label	= "X Rotation Friction",
					...
					Min		= 0,
					Max		= 100,
					Command = "easyprecision_XRotFric",
			...
			Panel:AddControl( "Slider",  { Label	= "Y Rotation Friction",
					...
					Min		= 0,
					Max		= 100,
					Command = "easyprecision_YRotFric",
			...
			Panel:AddControl( "Slider",  { Label	= "Z Rotation Friction",
					...
					Min		= 0,
					Max		= 100,
					Command = "easyprecision_ZRotFric",
					Description = "Rotation friction of advanced ballsocket in Z axis"}	 )

			Panel:AddControl("Checkbox", { Label = "Free Movement", Command = "easyprecision_FreeMov", Description = "Only lock relative rotation, not position?"  })
		end
		...
		if (mode == 8) then //slider
			Panel:AddControl( "Slider",  { Label	= "Slider Width",
					...
					Min		= 0.0,
					Max		= 10,
					Command = "easyprecision_width",
					Description = "Width of the slider black line (0 = invisible)"}	)

			Panel:AddControl("Checkbox", { Label = "Turn Off Minor Slider Stabilisation", Command = "easyprecision_disablesliderfix", Description = "Fix being seperate X/Y/Z advanced ballsocket locks between the props.  This stops most spaz caused by rotation, but not spaz caused by displacement." })
		...
		if (showgenmenu == 1) then
			Panel:AddControl("Button", { Label = "\\/ General Tool Options \\/", Command = "easyprecision_generalmenu", Description = "Collapse menu"  })
		...
		if (user == 1) then
			params.Options[" 1 ->Normal<-"] = { easyprecision_setuser = "1" }
		else
			params.Options[" 1   Normal"] = { easyprecision_setuser = "1" }
		end
		if (user == 2) then
			params.Options[" 2 ->Advanced<-"] = { easyprecision_setuser = "2" }
		else
			params.Options[" 2   Advanced"] = { easyprecision_setuser = "2" }
		end
		if (user == 3) then
			params.Options[" 3 ->Experimental<-"] = { easyprecision_setuser = "3" }
		else
			params.Options[" 3   Experimental"] = { easyprecision_setuser = "3" }
		end
		...
		Panel:AddControl("ListBox", params)
			...
			Panel:AddControl("Checkbox", { Label = "Enable tool feedback messages?", Command = "easyprecision_enablefeedback", Description = "Toggle for feedback messages incase they get annoying"  })
			Panel:AddControl("Checkbox", { Label = "On = Feedback in Chat, Off = Centr Scrn", Command = "easyprecision_chatfeedback", Description = "Chat too cluttered? Can have messages centre screen instead"  })
			//Panel:AddControl("Checkbox", { Label = "Hide Menu Tips?", Command = "easyprecision_hidehints", Description = "Streamline the menu once you're happy with using the tool."  })
			Panel:AddControl("Checkbox", { Label = "Add Push/Pull to Undo List", Command = "easyprecision_nudgeundo", Description = "For if you're in danger of nudging somthing to where you can't reach it"  })
			Panel:AddControl("Checkbox", { Label = "Add Movement to Undo List", Command = "easyprecision_moveundo", Description = "So you don't have to secondary fire with nocollide to undo mistakes"  })
			Panel:AddControl("Checkbox", { Label = "Add Rotation to Undo List", Command = "easyprecision_rotateundo", Description = "So you can find the exact rotation value easier"  })
			Panel:AddControl("Button", { Label = "Restore Current Mode Default", Command = "easyprecision_defaultrestore", Description = "Collapse menu"  })
		else
			Panel:AddControl("Button", { Label = "-- General Tool Options --", Command = "easyprecision_generalmenu", Description = "Expand menu"  })
		end
	end
	local function easyprecision_defaults()
		local mode = LocalPlayer():GetInfoNum("easyprecision_mode", 3)
		if mode  == 1 then
			RunConsoleCommand("easyprecision_freeze", "1")
			RunConsoleCommand("easyprecision_autorotate", "1")
			RunConsoleCommand("easyprecision_ShadowDisable", "0")
			RunConsoleCommand("easyprecision_nocollideall", "0")
			RunConsoleCommand("easyprecision_physdisable", "0")
			RunConsoleCommand("easyprecision_allowphysgun", "0")
			RunConsoleCommand("easyprecision_entirecontrap", "0")
		elseif mode == 2 then
			RunConsoleCommand("easyprecision_rotation", "15")
			RunConsoleCommand("easyprecision_freeze", "1")
			RunConsoleCommand("easyprecision_entirecontrap", "0")
		elseif mode == 3 then
			RunConsoleCommand("easyprecision_rotate", "1")
			RunConsoleCommand("easyprecision_offset", "0")
			RunConsoleCommand("easyprecision_offsetpercent", "1")
			RunConsoleCommand("easyprecision_rotation", "15")
			RunConsoleCommand("easyprecision_freeze", "1")
			RunConsoleCommand("easyprecision_nocollide", "1")
			RunConsoleCommand("easyprecision_autorotate", "1")
			RunConsoleCommand("easyprecision_entirecontrap", "0")
		elseif mode == 4 then
			RunConsoleCommand("easyprecision_move", "1")
			RunConsoleCommand("easyprecision_rotate", "1")
			RunConsoleCommand("easyprecision_offset", "0")
			RunConsoleCommand("easyprecision_offsetpercent", "1")
			RunConsoleCommand("easyprecision_rotation", "15")
			RunConsoleCommand("easyprecision_freeze", "1")
			RunConsoleCommand("easyprecision_nocollide", "1")
			RunConsoleCommand("easyprecision_autorotate", "0")
			RunConsoleCommand("easyprecision_removal", "0")
			RunConsoleCommand("easyprecision_entirecontrap", "0")
			RunConsoleCommand("easyprecision_forcelimit", "0")
		elseif mode == 5 then
			RunConsoleCommand("easyprecision_move", "1")
			RunConsoleCommand("easyprecision_rotate", "1")
			RunConsoleCommand("easyprecision_offset", "0")
			...
			RunConsoleCommand("easyprecision_entirecontrap", "0")
			RunConsoleCommand("easyprecision_forcelimit", "0")
			RunConsoleCommand("easyprecision_torquelimit", "0")
			RunConsoleCommand("easyprecision_friction", "0")
		elseif mode == 6 then
			RunConsoleCommand("easyprecision_move", "1")
			RunConsoleCommand("easyprecision_rotate", "1")
			RunConsoleCommand("easyprecision_offset", "0")
			...
			RunConsoleCommand("easyprecision_torquelimit", "0")
		elseif mode == 7 then
			RunConsoleCommand("easyprecision_move", "0")
			RunConsoleCommand("easyprecision_rotate", "1")
			RunConsoleCommand("easyprecision_offset", "0")
			RunConsoleCommand("easyprecision_offsetpercent", "1")
			RunConsoleCommand("easyprecision_rotation", "15")
			...
			RunConsoleCommand("easyprecision_forcelimit", "0")
			RunConsoleCommand("easyprecision_torquelimit", "0")
			RunConsoleCommand("easyprecision_XRotMin", "0")
			RunConsoleCommand("easyprecision_XRotMax", "0")
			RunConsoleCommand("easyprecision_YRotMin", "0")
			RunConsoleCommand("easyprecision_YRotMax", "0")
			RunConsoleCommand("easyprecision_ZRotMin", "0")
			RunConsoleCommand("easyprecision_ZRotMax", "0")
			RunConsoleCommand("easyprecision_XRotFric", "0")
			RunConsoleCommand("easyprecision_YRotFric", "0")
			RunConsoleCommand("easyprecision_ZRotFric", "0")
			RunConsoleCommand("easyprecision_FreeMov", "1")
		elseif mode == 8 then
			RunConsoleCommand("easyprecision_move", "1")
			RunConsoleCommand("easyprecision_rotate", "1")
			RunConsoleCommand("easyprecision_offset", "0")
			...
			RunConsoleCommand("easyprecision_rotation", "15")
			RunConsoleCommand("easyprecision_freeze", "1")
			RunConsoleCommand("easyprecision_nocollide", "0")
			RunConsoleCommand("easyprecision_autorotate", "0")
			RunConsoleCommand("easyprecision_removal", "0")
			RunConsoleCommand("easyprecision_entirecontrap", "0")
			RunConsoleCommand("easyprecision_width", "1")
			RunConsoleCommand("easyprecision_disablesliderfix", "0")
		elseif mode == 9 then
			RunConsoleCommand("easyprecision_removal", "0")
			RunConsoleCommand("easyprecision_allowphysgun", "0")
			RunConsoleCommand("easyprecision_entirecontrap", "0")
		end
		easyprecision_updatecpanel()
	end
	concommand.Add("easyprecision_defaultrestore", easyprecision_defaults)

	local function easyprecision_genmenu()
		if (showgenmenu == 1) then
			showgenmenu = 0
		else
			showgenmenu = 1
	...
	concommand.Add("easyprecision_generalmenu", easyprecision_genmenu)

	function easyprecision_setmode(player, tool, args)
		if LocalPlayer():GetInfoNum("easyprecision_mode", 3) != args[1] then
			RunConsoleCommand("easyprecision_mode", args[1])
			timer.Simple(0.05, function() easyprecision_updatecpanel() end)
		end
	end
	concommand.Add("easyprecision_setmode", easyprecision_setmode)


	function easyprecision_setuser(player, tool, args)
		if LocalPlayer():GetInfoNum("easyprecision_user", 3) != args[1] then
			RunConsoleCommand("easyprecision_user", args[1])
			timer.Simple(0.05, function() easyprecision_updatecpanel() end)
		end
	end
	concommand.Add("easyprecision_setuser", easyprecision_setuser)


	function easyprecision_updatecpanel()
	...
	concommand.Add("easyprecision_updatecpanel", easyprecision_updatecpanel)
