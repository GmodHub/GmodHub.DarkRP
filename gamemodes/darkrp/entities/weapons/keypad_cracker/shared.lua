...
SWEP.Secondary.Ammo = ""
SWEP.BaseKeyCrackTime = 25
SWEP.KeyCrackTime = SWEP.BaseKeyCrackTime
...
function SWEP:PrimaryAttack()
	...
	if (not self.Owner:IsHired()) and self.Owner:GetTeamTable().hirable then return end

	if IsValid(self.Owner) and self.Owner:GetTeamTable().keypadcracktime then
		self.KeyCrackTime = self.BaseKeyCrackTime * self.Owner:GetTeamTable().keypadcracktime
	else
		self.KeyCrackTime = self.BaseKeyCrackTime
	end

	self.KeyCrackTime = self.Owner:CallSkillHook(SKILL_HACK, self.KeyCrackTime)
	...
	if(IsValid(ent) and tr.HitPos:Distance(self.Owner:GetShootPos()) <= 50 and ent.CanHack and ent:CanHack(self.Owner)) then
...
function SWEP:Succeed()
	...
	if SERVER and IsValid(ent) and (tr.HitPos:Distance(self.Owner:GetShootPos()) <= 50) and ent.Hack then
		ent:Hack(self.Owner)
...
function SWEP:Think()
	...
	if(self.IsCracking and IsValid(self.Owner)) then
		...
		if(not IsValid(tr.Entity) or tr.HitPos:Distance(self.Owner:GetShootPos()) > 50 or !tr.Entity.CanHack or !tr.Entity:CanHack(self.Owner)) then
			self:Fail()
...
if(CLIENT) then
	...
	function SWEP:PostDrawViewModel(vm, wep, pl)
		...
		cam.Start3D2D(vm:LocalToWorld(off), ang, 0.005)
			surface.SetDrawColor(rp.col.SUP.r, rp.col.SUP.g, rp.col.SUP.b, 25)
			for k, v in ipairs(self.Lines) do
				local diff = (st - v.st) / v.spd
				local x = -v.w + (diff * (w + v.w)) - 50

				if (x > w + 50) then
					table.insert(self.ToDel, k)
					continue
				end
			...
			surface.SetMaterial(kaliLogo)
			surface.SetFont("ui.40")
			if (!self.IsCracking) then
				self.WordOfTheDay = nil
				if (not self.Owner:IsHired()) and self.Owner:GetTeamTable().hirable then
					surface.SetDrawColor(255, 0, 0)
				end
				surface.DrawTexturedRect((w - 220) * 0.5, (h - 175) * 0.5, 220, 175)
				surface.SetTextColor(75, 75, 75)
				local txt = (self.Owner:IsHired() or not self.Owner:GetTeamTable().hirable) and "Ready to hack!" or "Unable to hack: not hired!"
				local tw = surface.GetTextSize(txt)
				surface.SetTextPos((w - tw) * 0.5, 235)
				surface.DrawText(txt)
			else
				if (!self.WordOfTheDay) then
					if (math.random(100) % 5 < 3) then -- verb noun
						self.WordOfTheDay = table.Random({"Hacking", "Pwning", "RIP'n'", "Rekking", "DDoSing", "Nulling", "Breaching", "Sploiting"}) .. " " .. table.Random({"Noobs", "Pad", "Nerds", "Bitches", "Luas"}) .. "!"
					else -- other meme
						self.WordOfTheDay = table.Random({"#Hackerman", "pad:Crack()", "#BigBrotherCalls", "Get Gud Kid", "SELECT `passKey`", "H4CK1NG", "sudo rm -rf /", "#hacker.Squad", "Modifying Params", "Hacker Shmacker"})
					end
				end

				self.StartCrack = self.StartCrack or CurTime()
				surface.DrawTexturedRect((w - 220) * 0.5, (h - 175) * 0.3, 220, 175)
				if (self.EndCrack) then
					local x, y = (w / 2) - 175, 230
