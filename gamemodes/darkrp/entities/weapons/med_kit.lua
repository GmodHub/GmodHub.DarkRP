...
function SWEP:PrimaryAttack()
	...
	self:SetNextPrimaryFire(CurTime() + self.Owner:CallSkillHook(SKILL_MEDIC, self.Primary.Delay))
	...
	self.Owner.MedkitTimeout[ent:SteamID64()] = self.Owner.MedkitTimeout[ent:SteamID64()] or {
		Healed = 0,
		ExpireAt = CurTime() + 15,
		BlockUntil = 0
	}

	local inf = self.Owner.MedkitTimeout[ent:SteamID64()]
	...
	if (inf.BlockUntil < CurTime()) then -- Only accumulate if not blocked
		...
		if (inf.Healed >= 50) then -- Karma reward
