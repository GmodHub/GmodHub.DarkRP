...
if (CLIENT) then
	...
	function ENT:Draw()
		...
		cam.Start3D2D(pos, Angle(0, LocalPlayer():EyeAngles().yaw - 90, 90) , 0.065)
			draw.SimpleTextOutlined(self.LabType, '3d2d', 0, -350, color_text, TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_textoutline)
		cam.End3D2D()

		local ang = self:GetAngles()
		ang:RotateAroundAxis(ang:Up(), 63)

		cam.Start3D2D(pos + (self:GetUp() * 14.4) - (self:GetForward() * 3) - (self:GetRight()* 2), ang, 0.025)
