...
function ENT:PhysicsCollide(data, physnum)
	...
	if IsValid(ent) then
		...
		if ent:IsPlayer() and (not ent.IsTased) then
			if (tase) then
				if (ent.TaserResistance and ent.TaserResistance > 0) then
					local rand = math.random(1000)
					if (rand <= ent.TaserResistance * 1000) then
						if IsValid(self.Weapon) and IsValid(self.Weapon.Owner) then
							rp.Notify(self.Weapon.Owner, NOTIFY_ERROR, term.Get('PlayerTaserResistance'), ent)
						end

						rp.Notify(ent, NOTIFY_SUCCESS, term.Get('PlayerTaserResistanceSelf'))
						self:Remove()
						tase = false
					end
				end

				ent.TaserResistance = (ent.TaserResistance or 0) + 0.15
				timer.Simple(time.resistdrop, function()
					if (IsValid(ent)) then
						ent.TaserResistance = ent.TaserResistance - 0.15
					end
				end)
			end

			if (tase) then
				if (not ent:IsWanted()) and self.Owner:IsCP() then
			...
			if (tase) then
				hook.Call('PlayerTasePlayer', nil, self.Shooter, ent)
				...
				hook.Call('PlayerTasePlayerFail', nil, self.Shooter, ent)
			end
		end
...
hook.Add('GravGunPickupAllowed', 'rp.taser.GravGunPickupAllowed', function(pl, ent)
	if (ent:GetClass() == 'ent_taser_hook') then
		return false
	end
end)
