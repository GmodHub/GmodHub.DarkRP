...
function ENT:Think()
	...
	if (lookEnt and lookEnt == self and self:GetStatus() == self.Status_None) then
		self.LookTime = self.LookTime + FrameTime()

		if (self.LookTime >= 1) then
			net.Start('rp.keypad.FaceID')
...
function ENT:Draw()
	...
	if (IsValid(LP)) then
		...
		cam.Start3D2D(pos, ang, 0.05)
			...
			-- neutral = 0 or nil, denied = 1, granted = 2
			local status = self:GetStatus() or 0

			if (self:GetFaceID() != true) then
				self.FaceIDTime = nil
				...
				if (lookEnt and lookEnt == self) and self:GetFaceIDEnabled() then
					surface.SetMaterial(mat_faceID)
					surface.SetDrawColor(255, 255, 255, (SysTime() % .5) >= .25 and 255 or 0)
					surface.DrawTexturedRect(X + 6, Y + 44, 10, 10)
				end
			else
				self.FaceIDTime = math.Min((self.FaceIDTime or 0) + FrameTime(), 0.75)
				local mul = self.FaceIDTime / 0.75

				surface.SetDrawColor(50 + mul * 205, 75 + mul * 180, 50 + mul * 205, 255)
				surface.DrawRect(X+5, Y+5, 90, 50)

				surface.SetMaterial(mat_faceID)
				if (status == 2) then
					surface.SetDrawColor(50, 130, 255, 255 * mul)
				else
					surface.SetDrawColor(0, 0, 0, 255 * mul)
				end
				surface.DrawTexturedRect(X + 5 + (mul * 20), Y+5, 90 - mul * 40, 50)
			end
...
function ENT:EnterKey(LP, k)
	...
	if (k <= 9) then
		net.Start("rp.keypad.Number")
			net.WriteEntity(self)
			net.WriteUInt(k, 4)
		net.SendToServer()
	else
		local isSubmit = k == KEY_ENTER or k == KEY_PAD_ENTER
		net.Start("rp.keypad.Func")
			net.WriteEntity(self)
...
hook.Add("Think", "Keypad_Keypad", function()
	...
	if (IsValid(LP)) then
		...
		if (IsValid(tr.Entity) and tr.Entity:GetClass() == "pad_containing_some_keys_for_fading_doors") then
			lookEnt = tr.Entity
			traceRes = tr
...
hook.Add("KeyRelease", "Keypad_ReleaseCheck", function(LP, k)
